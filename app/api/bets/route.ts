...
export async function POST(request:Request){
 ...
 const url=new URL(`https://api.the-odds-api.com/v4/sports/${b.sportKey}/odds/`);url.search=new URLSearchParams({apiKey:process.env.ODDS_API_KEY||"",bookmakers:"pinnacle",markets:"h2h,spreads,totals",oddsFormat:"decimal",dateFormat:"iso"}).toString();
 const games=await (await fetch(url,{cache:"no-store"})).json();const game=Array.isArray(games)?games.find((g:any)=>g.id===b.eventId):null;const book=game?.bookmakers?.find((x:any)=>x.key==="pinnacle")||game?.bookmakers?.[0];const market=book?.markets?.find((m:any)=>m.key===b.marketKey);const outcome=market?.outcomes?.find((o:any)=>o.name===b.selectionName&&(o.point??null)===(b.point??null));if(!game||!outcome)return Response.json({error:"オッズが更新されました。もう一度選んでください"},{status:409});
